...
export function RegisterWithReferralForm() {
  ...
  const validateUsernameUnique = async (username: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('usuarios')
        .select('id, usuario, estado_habilitado')
        .ilike('usuario', username) // Comparación insensible a mayúsculas/minúsculas
        .eq('estado_habilitado', true) // Solo considerar usuarios habilitados
        .maybeSingle()
            ...
            render={({ field }) => (
              <FormItem>
                ...
                {usernameValidation.message && !usernameValidation.isChecking && (
                  <div className={`text-xs ${usernameValidation.isDuplicate ? 'text-red-500' : 'text-green-600'}`}>
